...
unsigned char reverse(unsigned char byte){
    return (((byte & 0x01) << 7) | ((byte & 0x02) << 5)  | ((byte & 0x04) << 3) | ((byte & 0x08) << 1) | ((byte & 0x10) >> 1) | ((byte & 0x20) >> 3) |  ((byte & 0x40) >> 5) | ((byte & 0x80) >> 7)); 
}

unsigned char lCounter(unsigned char byte){
    unsigned char lfsr = (byte << 1) | (((byte & 0x40) >> 6) ^ ((byte & 0x20) >> 5));
    lfsr &= 0x7f;
    return lfsr;
}
